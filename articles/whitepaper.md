# Solving Singularities in Neural QMC: A 330x Compression of DeepMind's FermiNet using Physics-Designed KANs
**How overcoming the $r \to 0$ singularity and reviving LCAO within Kolmogorov-Arnold Networks created an interpretable, Neuro-Symbolic physics engine to ground Agentic AI and LLMs.**


## TL;DR

**What if your continuous 3D neural network never encountered a singularity, never diverged at infinity, and required zero pre-training to reach practical accuracy? (e.g., $r \to 0$ causes $1/r$ to explode to NaN, instantly destroying the Adam optimizer)**

In this article, we introduce an architecture that solves the fundamental "Origin Singularity" (the $r \to 0$ collapse) in continuous 3D machine learning. By mapping 3D Cartesian coordinates to a 4D epsilon-manifold and combining them with a **Physics-Designed KAN (PD-KAN)**, we demonstrated:
1. **Numerical Stability**: Eliminating the $r \to 0$ origin singularity with zero infinite gradients.
2. **330x Parameter Compression**: Shrinking DeepMind's FermiNet from 437,200 down to just 1,328 parameters.
3. **Zero-Shot Initialization**: Bypassing massive pre-training via exact physical mapping.
...
Instead of generic splines, we adopted **Atomic Orbital Bases**, which adhere to the laws of quantum physics, as the basis functions for our KAN edges. Mathematically, it takes this form:
...
2. **Tabula Rasa Autonomous Discovery (Angular):** In stark contrast, the angular component starts from a completely blank slate ($W=0$). We do *not* hardcode spherical harmonics. The AI is given only raw Cartesian monomials and must autonomously assemble them to "invent" polarized orbitals (e.g., $p_z$) and chemical hybridization purely through gradient descent.

By plugging PD-KAN into the FermiNet infrastructure, we achieve a zero-shot initialization paradigm. The network starts with a physically safe state at Step 0, bypassing the heavy pre-training phase entirely while retaining the freedom to discover profound chemical phenomena autonomously.
...
2. **Physical Basis Evaluation (PD-KAN)**
   ...
   - **Angular**: The 4D embedding feeds into an Angular KAN, assembling Cartesian monomials with learnable weights (*Tabula Rasa*).
   ```mermaid
   graph LR
...
To demonstrate the power of PD-KAN, I need to make a confession: I originally intended to run this on top of FermiNet using its notorious K-FAC optimizer. However, the official repository has somewhat fossilized, and K-FAC is currently broken under recent JAX updates. Unable to fix the XLA dependencies myself, I was forced to fall back to the standard, much lighter Adam optimizer. 
...
*(For readers interested in a demonstration of how we opened this "Glass Box" to mathematically prove that the AI autonomously discovered the Restricted Hartree-Fock (RHF) spatial symmetry without human constraints, please refer to the detailed [Interpretability Demo (Autonomous RHF Discovery)](https://github.com/Samu-Physicist/FermiKAN/blob/main/H2_interpretability_demo.ipynb) in the project repository.)*

---

## 8. Conclusion and Broader Impact: AI as an Exponent on Human Intelligence

Today, as the computational cost of AI models continues to skyrocket, society rightly questions whether this massive expenditure of energy and infrastructure is justified. 

Through this project, I realized something profound. I am just an ordinary R&D researcher at a chemical manufacturer—a human baseline, not a giant. Let's call it a base of $1.1$. However, by pairing my domain knowledge in theoretical physics with modern AI (including brainstorming and pair programming with Gemini) and borrowing the FermiNet architecture, I was able to suggest a new paradigm in computational chemistry: learnable physical basis sets. 

We do not pour electricity into data centers to replace human intelligence. If our human baseline drops to $0.9$, applying the immense exponent of AI will only drive our collective capability down to zero. But as long as we maintain a base of $1.1$ or more, AI acts as an infinite exponent that drives human potential towards infinity, empowering us to solve the grand challenges of our time.
